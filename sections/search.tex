...
For this prover we will not resort to reflection or similar techniques: all of
the automation machinery will run in standard Agda.
To accomplish it we will leverage a common technique which we will describe
briefly here.
...
\end{agdalisting}
While it's not a particularly impressive example, it does nonetheless
demonstrate a principle we will exploit: closed terms will compute to a normal
form.
We do not have to manually apply each computation rule or pattern match in order
for the equation to type-check.
...
\section{Omniscience}
First we will explore the theoretical underpinning for search, and its relation
to finiteness.
The property we are looking for is effectively the ability to exhaustively
search through some type.
Consider the proof above in Equation~\ref{bool-assoc-auto-proof}: we know the
proof could be automatic because we can check every configuration of the values
of \(x\), \(y\), and \(z\).
