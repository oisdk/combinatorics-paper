...
\end{agdalisting}
We use the fact that \agdabot\; is itself a proposition to eliminate from under
the truncation.
\begin{agdalisting}
  \ExecuteMetaData[agda/Cardinality/Finite/Cardinal.tex]{no-gap-card-bishop-proof}
\end{agdalisting}
...
the same result.
The sort we use is a standard insertion sort:
\begin{multicols}{2}
  \begin{agdalisting}
...
\begin{multicols}{2}
  ...
  \end{agdalisting}
\end{multicols} \vspace{-\baselineskip}
We've introduced permutations here: we take their definition from
...
And indeed this means that the circle is manifestly enumerable. \todo{This uses
  a bunch of cubical primitives!}
\begin{agdalisting}
