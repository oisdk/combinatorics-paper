...
module Algebra.Construct.Free.CommutativeMonoid where
...
%<*elim-fn>
\begin{code}
 module _ {a p} {A : Type a} (P : ⟅ A ⟆ → Type p)
       (f : ∀ x xs → P xs → P (x ∷ xs))
       (z : P [])
       (s : ∀ {xs} → isSet (P xs))
       (c : ∀ x y xs pxs →
  ...
  elim : ∀ xs → P xs
  elim []                     = z
  elim (x ∷ xs)               = f x xs (elim xs)
  elim (com x y xs i)         = c x y xs (elim xs) i
  elim (trunc xs ys x y i j)  =
    isOfHLevel→isOfHLevelDep {n = 2}
      (λ xs → s {xs})
      (elim xs) (elim ys)
      (cong elim x) (cong elim y)
      (trunc xs ys x y)
      i j
\end{code}
%</elim-fn>
...
%</elim>
\begin{code}
syntax elim-syntax A (λ xs → Pxs) =
  ⟅ xs  ⟅ A ⟆↝ Pxs ⟆

record ⟦_⇒_⟧ {a ℓ} (A : Type a) (P : ⟅ A ⟆ → Type ℓ) : Type (a ℓ⊔ ℓ) where
