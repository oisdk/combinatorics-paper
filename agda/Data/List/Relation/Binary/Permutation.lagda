...
swapₚ-to _ _ _ _ (f0 , x≡x₁) = f1 , x≡x₁
swapₚ-to _ _ _ _ (f1 , x≡x₂) = f0 , x≡x₂
...
%</tail>
\begin{code}
equivPerm : ∀ {a} {A : Type a} → Equivalence (List A) a
equivPerm {A = A} =
  record { _≋_ = _↭_
         ; sym = λ {xs} {ys} → symₚ {A = A} {xs = xs} {ys = ys}
         ; refl = λ {xs} → reflₚ {A = A} {xs = xs}
         ; trans = λ {xs} {ys} {zs} → transₚ {A = A} {xs = xs} {ys = ys} {zs = zs}
         }
