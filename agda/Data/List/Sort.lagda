...
insert-sorts x xs lb≤x Pxs = ⟦ step ⟧⇓ xs Pxs x lb≤x
  where
  step : xs ⦂List E ⇘ (∀ {lb} → SortedFrom lb xs → ∀ x → lb ≤⌊ x ⌋ → SortedFrom lb (insert x xs))
  ⟦ step ⟧[] _ x lb≤x = lb≤x , tt
  ⟦ step ⟧ y ∷ xs ⟨ k ⟩ (lb≤y , Sxs) x lb≤x with x ≤? y
  ... | inl x≤y = lb≤x , x≤y , Sxs
  ... | inr y≤x = lb≤y , k Sxs x y≤x

sort-sorts : ∀ xs → Sorted (sort xs)
sort-sorts = ⟦ elim tt (λ x xs → insert-sorts x (sort xs) tt) ⟧⇓

open import Relation.Binary.Equivalence.Reasoning (equivPerm {A = E}) public

insert-perm : ∀ x xs → insert x xs ↭ x ∷ xs
insert-perm = λ x → ⟦ step x ⟧⇓
  where
  step : ∀ x → xs ⦂List E ⇘ insert x xs ↭ x ∷ xs
  ⟦ step x ⟧[] = reflₚ
  ⟦ step x ⟧ y ∷ xs ⟨ Pxs ⟩ with x ≤? y
  ... | inl x≤y = consₚ x reflₚ
  ... | inr y≤x =
    y ∷ insert x xs ≋⟨ consₚ y Pxs ⟩
    y ∷ x ∷ xs ≋⟨ swapₚ y x xs ⟩
    x ∷ y ∷ xs ∎
...
sort-perm (x ∷ xs) =
  sort (x ∷ xs) ≋⟨⟩
  insert x (sort xs) ≋⟨ insert-perm x (sort xs) ⟩
  x ∷ sort xs ≋⟨ consₚ x (sort-perm xs) ⟩
  x ∷ xs ∎
