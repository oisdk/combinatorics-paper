...
record PartialOrder {ℓ₁} (𝑆 : Type ℓ₁) ℓ₂ : Type (ℓ₁ ℓ⊔ ℓ-suc ℓ₂) where
  infix 4 _≤_
  field
    _≤_ : 𝑆 → 𝑆 → Type ℓ₂
    refl : Reflexive _≤_
...
record TotalOrder {ℓ₁} (𝑆 : Type ℓ₁) ℓ₂ : Type (ℓ₁ ℓ⊔ ℓ-suc ℓ₂) where
  field
...
record Equivalence {ℓ₁} (𝑆 : Type ℓ₁) ℓ₂ : Type (ℓ₁ ℓ⊔ ℓ-suc ℓ₂) where
  infix 4 _≋_
  field
    _≋_ : 𝑆 → 𝑆 → Type ℓ₂
    sym   : ∀ {x y} → x ≋ y → y ≋ x
...
record LinearOrder {ℓ₁} (𝑆 : Type ℓ₁) ℓ₂ : Type (ℓ₁ ℓ⊔ ℓ-suc ℓ₂) where
  field
    _<_ : 𝑆 → 𝑆 → Type ℓ₂
  _≮_ : 𝑆 → 𝑆 → Type ℓ₂
  x ≮ y = ¬ (x < y)
  ...
  trans : ∀ {x y z} → x < y → y < z → x < z
  trans {x} {y} {z} x<y y<z with comp z x<y
  trans {_} {_} {_} x<y y<z | inl x<z = x<z
  trans {_} {_} {_} x<y y<z | inr z<y = ⊥-elim (asym y<z z<y)
\end{code}
