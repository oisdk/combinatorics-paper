...
\begin{romdefinition}[Membership of \(\mathcal{K}\)]
  ...
  \begin{equation}
    \begin{alignedat}{2}
      x \in&& \; []                      &= \bot \\
      x \in&& \; y \dblcolon \mathit{ys} &= \lVert x \equiv y \uplus x \in \mathit{ys} \rVert
    \end{alignedat}
  \end{equation}
...
\begin{romlemma} \label{split-enum-is-split-surj}
  \begin{equation}
    \mathcal{E}!(A) \equiv \Sigma (n : \mathbb{N}) , \left( \mathbf{Fin}(n) \twoheadrightarrow ! \; A \right)
  \end{equation}
\end{romlemma}
...
\begin{proof}
  \begin{align*}
    \mathcal{E}!(A)
    & \equiv \Sigma (\mathit{xs} : \textbf{List}(A)) , \Pi {(x : A)} , x \in \mathit{xs}
    & \text{Definition of } \mathcal{E}!
    \\
    & \equiv \Sigma (\mathit{xs} : \textbf{List}(A)) , \Pi {(x : A)} , \text{fib}_{\text{snd}(\mathit{xs})}(x)
    & \text{Definition of } \in
    \\
    & \equiv \Sigma (\mathit{xs} : \textbf{List}(A)) , \text{sp-surj}(\text{snd}(\mathit{xs}))
    & \text{Definition of sp-surj}
    \\
    & \equiv \Sigma (\mathit{xs} : \llbracket \mathbb{N} , \mathbf{Fin} \rrbracket (A)) , \text{sp-surj}(\text{snd}(\mathit{xs}))
    & \text{Definition of } \mathbf{List}
    \\
    & \equiv \Sigma (\mathit{xs} : \Sigma (n : \mathbb{N}) , \Pi (i : \mathbf{Fin}(n)) , A) , \text{sp-surj}(\text{snd}(\mathit{xs}))
    & \text{Definition of } \llbracket \wc \rrbracket
    \\
    & \equiv \Sigma (n : \mathbb{N}) , \Sigma (f : \mathbf{Fin}(n) \rightarrow A) , \text{sp-surj}(f)
    & \text{Reassociation of } \Sigma
    \\
    & \equiv \Sigma (n : \mathbb{N}) , ( \mathbf{Fin}(n) \twoheadrightarrow ! \; A )
...
\begin{proof}
  \begin{align*}
    \mathcal{B}(A)
    & \equiv \Sigma (\mathit{xs} : \textbf{List}(A)) , \Pi {(x : A)} , x \inunique \mathit{xs}
    & \text{Definition of } \mathcal{B}
    \\
    & \equiv \Sigma (\mathit{xs} : \textbf{List}(A)) , \Pi {(x : A)} , \text{isContr} \left(\text{fib}_{\text{snd}(\mathit{xs})}(x)\right)
    & \text{Definition of } \inunique
    \\
    & \equiv \Sigma (\mathit{xs} : \textbf{List}(A)) , \text{isEquiv}(\text{snd}(\mathit{xs}))
    & \text{\cite[theorem 4.4.5]{hottbook}}
    \\
    & \equiv \Sigma (\mathit{xs} : \llbracket \mathbb{N} , \mathbf{Fin} \rrbracket (A)) , \text{isEquiv}(\text{snd}(\mathit{xs}))
    & \text{Definition of } \mathbf{List}
    \\
    & \equiv \Sigma (\mathit{xs} : \Sigma (n : \mathbb{N}) , \Pi (i : \mathbf{Fin}(n)) , A) , \text{isEquiv}(\text{snd}(\mathit{xs}))
    & \text{Definition of } \llbracket \wc \rrbracket
    \\
    & \equiv \Sigma (n : \mathbb{N}) , \Sigma (f : \mathbf{Fin}(n) \rightarrow A) , \text{isEquiv}(f)
    & \text{Reassociation of } \Sigma
    \\
    & \equiv \Sigma (n : \mathbb{N}) , ( \mathbf{Fin}(n) \simeq \; A )
    & \text{Definition of } \twoheadrightarrow!
  \end{align*}
  \qed
\end{proof}
...
membership proof.
Here we have propositionally truncated it.
