...
something.
Finiteness is one of the prime examples which demonstrates this phenomenon: to
show that some type is finite, we might provide a bijection with another
finite set, or a surjection from some finite type.
We could alternatively say that a type is finite if any collection of its
elements larger than some bound contains duplicates, or if any stream of its
elements contains duplicates.

Classically, all of these ways to prove finiteness end up proving the same
