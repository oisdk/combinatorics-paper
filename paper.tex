...
In particular, we are interested in two different aspects of each predicate for
finiteness: how informative it is (i.e. how many properties of the underlying
type we can derive from a proof that it is finite), and how restrictive it is
(i.e. how many types conform to its particular variety of finiteness).
Once we explore each of five predicates along these two axes, we will then
construct a category out of one of them (decidable Kuratowski finiteness), and
show that it forms a \(\Pi\)-pretopos.
We will then take a brief detour into predicates for countably infinite types,
and compare its landscape to that of the finite predicates, before jumping into
a practical application of the work: a library for proof search over finite
domains.
