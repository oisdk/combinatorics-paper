...
\end{romtheorem}
\subsection{Manifest Enumerability and Kuratowski Finiteness}
\begin{romlemma}
  Any manifest enumerable type is Kuratowski finite.
\end{romlemma}
\subsection{Cardinal Finiteness and Kuratowski Finiteness}
\begin{romtheorem} \label{kuratowski-cardinal}
  \begin{equation}
    \mathcal{K}^f(A) \times \text{Discrete}(A) \simeq \mathcal{C}(A)
  \end{equation}
\end{romtheorem}
\section{Topos}
...
\end{romlemma}
\begin{proof}
  We prove this by showing an isomorphism with \(\mathbf{Fin}(2)\).
  \begin{align*}
    \text{false} & \leftrightarrow 0 \\
    \text{true} & \leftrightarrow 1
  \end{align*}
  \qed
\end{proof}
