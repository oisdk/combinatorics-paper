...
\todo[inline]{redo these last two paragraphs}
...
\begin{rm-definition}[Kuratowski Finite Subset]
  \(\mathcal{K}(A)\) is the type of Kuratowski-finite subsets of \(A\).
  \begin{equation}
    \begin{aligned}
      \mathcal{K}(A) =&
             \; \eta : A \rightarrow \mathcal{K}(A) ; \\
      \vert& \; \emptyset : \mathcal{K}(A) ; \\
      \vert& \; \wc \cup \wc : \mathcal{K}(A) \rightarrow \mathcal{K}(A) \rightarrow \mathcal{K}(A) ; \\
      \vert& \; \text{associative} : \Pi (\mathit{xs}, \mathit{ys}, \mathit{zs}: \mathcal{K}(A)) , (\mathit{xs} \cup \mathit{ys}) \cup \mathit{zs} \equiv \mathit{xs} \cup (\mathit{ys} \cup \mathit{zs}) ; \\
      \vert& \; \text{identity} : \Pi (\mathit{xs}: \mathcal{K}(A)) , \emptyset \cup \mathit{xs} \equiv \mathit{xs} ; \\
      \vert& \; \text{commutative} : \Pi (\mathit{xs}, \mathit{ys}: \mathcal{K}(A)) , \mathit{xs} \cup \mathit{ys} \equiv \mathit{ys} \cup \mathit{xs} ; \\
      \vert& \; \text{idempotent} : \Pi (\mathit{xs}: \mathcal{K}(A)) , \mathit{xs} \cup \mathit{xs} \equiv \mathit{xs} ; \\
...
Next, we need a way to say that a type is Kuratowski finite, rather than just
some subset.
...
Split enumerability is quite a strong predicate, implying both decidable
