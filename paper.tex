...
\end{enumerate}
In other words, it makes sense constructively to talk about a function of type
\(\mathbf{Finite}(A) \rightarrow \mathbb{N}\), which takes a \emph{proof} of the
finiteness of \(A\) and returns \(A\)'s cardinality.

Such functions are perhaps more concrete than this description suggests:
aside from being interesting in its own right, much of the attention given to
constructive mathematics comes from its perceived association with
computers (themselves a burgeoning topic of study, bound to catch on some day). 
Indeed many constructive proofs are in fact computer programs in disguise (and
vice-versa \cite{wadlerPropositionsTypes2015}): the functions referred to above
correspond to honest-to-goodness functions in a programming language (Agda
\cite{vezzosiCubicalAgdaDependently2019} in our case) which can be run on a
computer to produce real output.

All of this added information in our proofs is interesting and useful, yes, but
it also necessitates some extra caution.
We will see cases where we have provided \emph{too much} information in a
constructive proof, and we will need to hide it somehow.
...
This gives us the wrong category, however: for a type \(A\) with cardinality
\(n\), there are \(n!\) proofs that \(A\) is finite.
This means that we will have \(n!\) objects in the category for the set \(A\),
where we only want 1.
The solution is to hide some aspect of the proof: this is accomplished with
\emph{truncation}, which we will see in \todo{ref}.


For all of its benefits, constructive mathematics (especially dependently-typed
constructive mathematics on a computer) still suffers from many painful
drawbacks.
...
In this paper we are interested in two things: systematic generation of data,
and constructive finiteness.

First, let's explain what we mean by ``systematic generation of data''.
Consider QuickCheck \cite{claessenQuickCheckLightweightTool2011}: this
well-known Haskell library allows us to quickly and easily test
\ExecuteMetaData[haskell/PropertyTests.tex]{sort-stable}
