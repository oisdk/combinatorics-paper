...
\subsection{Proving Equivalence with Split Surjections}
Another, equivalent way to define ``finiteness'' is via a (split) surjection
from a finite prefix of the natural numbers.
Here, we will formalise this equivalence, by first proving an equivalence with a
container-based definition of split enumerability.
...
\begin{lemma} \label{container-split-enum-equiv}
  ...
  \end{equation}
  Membership for container-defined lists is a fiber.
  \begin{equation}
...
\begin{lemma} \label{container-split-enum-surj}
  ...
  some \(\mathbf{Fin}\).
  \begin{equation}
    \mathcal{E}!(A) \simeq \sum_{(n : \mathbb{N})} , \mathbf{Fin}(n) \twoheadrightarrow ! A
  \end{equation}
\end{lemma}
\begin{proof}
...
\end{proof}
This lets us prove our overall goal for this section.
\begin{theorem} \label{split-enum-surj}
  Split enumerability is equivalent to a split surjection from a finite prefix
  of the natural numbers.
\end{theorem}
\begin{proof}
  This follows from lemmas \ref{container-split-enum-equiv} and
  \ref{container-split-enum-surj}.
\end{proof}
...
\begin{proof}
  ...
  \todo[inline]{Rework this proof to rely on the split surjection equivalence
    from before}
\end{proof}
